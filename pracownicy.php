<?php
	...
	//$rezultat = $polaczenie->query("SELECT zm.id, zm.update_date, zm.id_klient, zm.id_usluga, zm.id_pracownik, usl.nazwa_uslugi, pra.imie, pra.nazwisko FROM zmiany zm INNER JOIN uslugi usl INNER JOIN pracownicy pra ON zm.id_pracownik=pra.id AND zm.id_usluga=usl.id ORDER BY zm.id DESC");
	//$rezultat1 = $polaczenie->query("SELECT zm.id, zm.id_klient, kl.id, kl.imie, kl.nazwisko FROM zmiany zm LEFT JOIN klienci1  kl ON zm.id_klient=kl.id ORDER BY zm.id DESC");
	$rezultat=$polaczenie->query("SELECT * FROM pracownicy0");
	}
				...
				<table id="customers" border="5px solid black">
					...
					<?php
						...
						{						
							...
							echo "<td>" . "<a href='raport.php?id_pracownika=$id_pracownika'><u>Stwórz raport</u></a>"."</td>";
							echo "</tr>";
						}
						echo "</table>";
